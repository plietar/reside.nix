...
#
# Rscript generate-r-universe.R /nix/store/60dzmyswlksx86xr1wkypwgq1zdxzjq7-source
...
with self; {
  ...
  monty = derive { name = "monty"; version = "0.4.0"; buildInputs = [cli rlang cpp11 callr cpp11 coda decor knitr mockery mvtnorm numDeriv pkgload posterior rmarkdown testthat withr]; };
  ...
  orderly = derive { name = "orderly"; version = "1.99.84"; buildInputs = [R6 cli diffobj fs gert httr2 jsonlite openssl rlang rstudioapi vctrs withr yaml DBI RSQLite callr jsonvalidate knitr mockery pkgload processx rmarkdown stringr testthat webfakes]; };
