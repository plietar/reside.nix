...
#
# Rscript generate-r-universe.R /nix/store/xkphdrw8p0f9fncm7j9aqbymfp22wacw-source
...
with self; {
  ...
  conan2 = derive { name = "conan2"; version = "1.9.102"; buildInputs = [ cli fs glue prettyunits rlang withr callr mockery openssl pkgdepends remotes renv testthat ]; };
  context = derive { name = "context"; version = "0.5.0"; buildInputs = [ crayon ids storr callr knitr mockery redux rmarkdown testthat withr ]; };
  ...
  didehpc = derive { name = "didehpc"; version = "0.4.0"; buildInputs = [ ]; };
  ...
  dust2 = derive { name = "dust2"; version = "0.3.22"; buildInputs = [ cli monty rlang cpp11 monty callr cpp11 decor fs glue knitr mockery numDeriv pkgbuild pkgload rmarkdown testthat withr ]; };
  ...
  eppasm = derive { name = "eppasm"; version = "0.8.4"; buildInputs = [ abind anclik binom epp fastmatch mvtnorm plyr readxl reshape2 vroom xml2 BH testthat knitr rmarkdown covr ]; };
  first90 = derive { name = "first90"; version = "1.7.1"; buildInputs = [ mvtnorm Matrix fastmatch Rcpp vroom BH Rcpp testthat ]; };
  frogger = derive { name = "frogger"; version = "0.1.0"; buildInputs = [ Rcpp RcppEigen beers eppasm glue Rcpp abind data.table dplyr knitr RcppEigen reshape2 rmarkdown SpectrumUtils testthat withr xml2 ]; };
  ...
  monty = derive { name = "monty"; version = "0.3.28"; buildInputs = [ R6 cli rlang cpp11 callr cpp11 coda decor knitr mockery mvtnorm numDeriv pkgload posterior rmarkdown testthat withr ]; };
  ...
  odin2 = derive { name = "odin2"; version = "0.3.26"; buildInputs = [ cli dust2 glue monty rlang decor deSolve fs knitr rmarkdown mockery numDeriv pkgload testthat withr ]; };
  orderly = derive { name = "orderly"; version = "1.6.1"; buildInputs = [ DBI R6 RSQLite crayon digest docopt fs gert ids withr yaml zip httr jsonlite knitr markdown mockery processx rmarkdown testthat vaultr ]; };
  ...
  orderly2 = derive { name = "orderly2"; version = "1.99.65"; buildInputs = [ R6 cli diffobj fs gert httr2 jsonlite openssl rlang rstudioapi withr yaml DBI RSQLite jsonvalidate knitr mockery pkgload processx rmarkdown stringr testthat ]; };
  orderlyweb = derive { name = "orderlyweb"; version = "0.1.15"; buildInputs = [ R6 gert httr jsonlite orderly progress zip mockery testthat ]; };
  ...
  provisionr = derive { name = "provisionr"; version = "0.1.14"; buildInputs = [ R6 curl drat prettyunits progress remotes rversions storr withr yaml testthat zip ]; };
  queuer = derive { name = "queuer"; version = "0.5.0"; buildInputs = [ R6 context ids lazyeval progress callr storr testthat thor ]; };
  ...
  spud = derive { name = "spud"; version = "0.1.7"; buildInputs = [ mockery testthat tibble withr getPass glue httr jsonlite xml2 ]; };
